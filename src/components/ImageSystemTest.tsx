...
import CompatibilityStatus from './CompatibilityStatus/CompatibilityStatus';
import { useCompatibility, useImageResourceAvailability } from '../hooks/useCompatibility';
...
const ImageSystemTest: React.FC = () => {
  ...
  const { availability } = useImageResourceAvailability(PET_TYPES[selectedPetType]);
  ...
  const runImageSystemTests = async () => {
    ...
    try {
      ...
      results.push({
        ...
        details: report ? `支持功能: ${Object.values(report).filter(Boolean).length}/5` : '检查失败'
      });
      ...
      // 测试3: 占位符图像生成
      try {
        const { generateDefaultPetPlaceholders } = await import('../utils/placeholderImageGenerator');
    ...
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          >
            {currentPetType && Object.entries(currentPetType.expressions).map(([key, expression]) => (
              <option key={key} value={key}>{expression.name}</option>
            ))}
          </select>
        </div>
      ...
      {availability && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ 
            padding: '10px', 
            backgroundColor: availability.percentage > 50 ? '#e8f5e8' : '#fff3e0',
            borderRadius: '4px'
          }}>
            <p>可用: {availability.available}/{availability.total} ({availability.percentage.toFixed(1)}%)</p>
              ...
              <div 
                ...
                style={{ 
                  ...
                  backgroundColor: 
                    ...
                    result.status === 'WARN' ? '#fff3e0' :
                    result.status === 'FAIL' ? '#ffebee' : '#f0f0f0',
                  borderRadius: '4px'
                }}
              >
                <strong>[{result.status}]</strong> {result.test}: {result.details}
              </div>
            ))}
          </div>
        </div>
