...
} from "@/config/performanceConfig"; // Import performance config
import { useImagePreloader } from "@/hooks/utils/useImagePreloader"; // Import image preloader
// 开发环境导入统一测试工具
if (process.env.NODE_ENV === "development") {
  void import("@tests/utils/unifiedTestHelpers");
}
...
const PetWindow: React.FC = () => {
  ...
  const interactionFeedbackResult = useInteractionFeedback({
    ...
    clickFeedbackDuration:
      PERFORMANCE_CONFIG.INTERACTION.CLICK_FEEDBACK_DURATION,
  ...
  // 气泡位置引用
  const bubblePositionRef = useRef<{ top: number; left: number }>({
    top: -55,
    left: 35,
  }); // Adjusted bubble position
  ...
  const currentPetType: PetType =
    PET_TYPES[currentPetTypeId] || PET_TYPES["default"];
  ...
  const expressionConfig = useMemo(() => {
    ...
    if (currentAnimation) {
      return (
        currentPetType.expressions["happy"] ||
        currentPetType.expressions["normal"]
      ); // Temporary happy during animation
    }
    ...
    if (status.mood > 80 && currentPetType.expressions["happy"])
      return currentPetType.expressions["happy"];
    if (status.mood < 20 && currentPetType.expressions["sad"])
      return currentPetType.expressions["sad"];
    if (lowStatusFlags.energy && currentPetType.expressions["sleepy"])
      return currentPetType.expressions["sleepy"];
    ...
    if (status.mood > 70) {
      ...
      if (
        status.level >= 15 &&
        rand > 0.85 &&
        currentPetType.expressions["level15"]
      )
        return currentPetType.expressions["level15"];
      if (
        status.level >= 10 &&
        rand > 0.75 &&
        currentPetType.expressions["level10"]
      )
        return currentPetType.expressions["level10"];
      if (
        status.level >= 5 &&
        rand > 0.65 &&
        currentPetType.expressions["level5"]
      )
        return currentPetType.expressions["level5"];
    }
  ...
  const wrappedHandleMouseEnter = useCallback(
  ...
  const wrappedHandleMouseLeave = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      try {
  ...
  // 增强的鼠标按下处理
  const enhancedHandleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      try {
        // 首次交互保护 - 修复首次点击隐藏问题
        if (!hasInteracted) {
  ...
  // 增强的鼠标抬起处理 (结合拖拽和交互反馈)
  const enhancedHandleMouseUp = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      try {
        // 记录用户交互
        hapticFeedback.recordUserInteraction();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transition: isDragging
            ? "none"
            : "all 0.15s cubic-bezier(0.4, 0, 0.2, 1)", // 与交互反馈保持一致
        }}
      >
        <div
          className="pet-body"
          style={
            currentPetType.modelType === "emoji"
              ? {
      ...
      {/* Level Up Animation */}
      {levelUpAnimation && (
        <div className="level-up-animation">升级! {status.level}</div>
      )}
